...
(function () {
  ...
  function grantReward(type, meta) {
    ...
    if (type === 'mult') {
      ...
      CK.mult = (CK.mult || 1) + (meta.mult || 0);
      if (window.__CKEX) window.__CKEX.megaCount = (window.__CKEX.megaCount || 0) + (meta.mega || 0);
      if (window.ckToast) window.ckToast('НАГРАДА: +' + meta.mult + 'x множ и +' + meta.mega + ' МЕГА!');
    } else if (type === 'super_reward') {
      CK.mult = (CK.mult || 1) + (meta.mult || 0);
      if (window.__CKEX) window.__CKEX.superCount = (window.__CKEX.superCount || 0) + (meta.superCnt || 0);
      FEAT.stats.permGoldBonus = (FEAT.stats.permGoldBonus || 0) + (meta.permGold || 0);
      if (window.ckToast) window.ckToast('🔥 ТИТУЛ ДЕМИУРГА ПОЛУЧЕН! +' + meta.mult + 'x и ' + meta.superCnt + ' СУПЕР!');
  ...
  /* Блок 8: Сезонный Пропуск (100 Уровней, Прогрессивная сложность) */
  var SEASON_LEVELS_COUNT = 100;

  function getXpForLevel(lvl) {
    // Прогрессивный экспоненциально-линейный рост требуемого опыта
    return Math.floor(1200 + (lvl * 350) + Math.pow(lvl, 1.65) * 45);
  }

  function getTotalXpForLevel(targetLvl) {
    var total = 0;
  ...
  function getSeasonRewards(lvl) {
    ...
    // ЭТАП 1: УРОВНИ 1 - 49 (ПРЕМИУМ: солидные ресурсы, четные бусты 4м-8м, ядра и постоянный доход)
    if (lvl < 50) {
      var mod = lvl % 5;
      if (mod === 0) {
    ...
    // ЭТАП 2: УРОВНИ 50 - 99 (ПРЕМИУМ: элитные артефакты, Призмы, Новы, Пустота, МЕГА-очки, четные бусты 10м-20м)
    var highLvl = lvl - 50;
    var hMod = lvl % 5;
    ...
    if (hMod === 0) {
      // Уровни 55, 60, 65, 70, 75, 80, 85, 90, 95: Элитный синтез и МЕГА
      ...
      var premMin = 10 + ((highLvl % 3) * 4); // 10м, 14м, 18м
      return {
  ...
  function renderSeasonPassModal(box) {
    ...
    premBtn.addEventListener('click', function () {
      ...
      FEAT.megaSpentOnSkills = (Number(FEAT.megaSpentOnSkills) || 0) + 5;
      FEAT.season.premium = true;
      saveState();
      if (window.sfxBuy) window.sfxBuy();
      if (window.ckToast) window.ckToast('ПРЕМИУМ ПРОПУСК АКТИВИРОВАН!');
    ...
    for (var l = 1; l <= SEASON_LEVELS_COUNT; l++) {
      ...
      var isCur = curLvl === l;
      rowsH += '<div class="ck-pass-row ' + (isCur ? 'current-lvl' : '') + '">' +
        '<div class="ck-pass-lvl-badge" style="' + (isCur ? 'color:#ffd76a;text-shadow:0 0 10px #fbbf24;transform:scale(1.1);' : '') + '">УР.' + l + '</div>' +
        '<div class="ck-pass-reward-card">' +
          '<span>' + rews.free.label + '</span>' +
          '<button class="ck-pass-claim" data-claim-free="' + l + '" ' + ((!isUnlocked || isFreeClaimed) ? 'disabled' : '') + '>' +
            (isFreeClaimed ? '✓' : 'ВЗЯТЬ') +
